...
league_name = "O Connells All Stars"  # Name of your league, must be exact match, can be done via env or hardcoded
start_budget = 50_000_000               # Starting budget of your league, used to calculate current budgets of other managers
league_start_date = "2026-01-02"        # Start date of your league, used to filter activities, format: YYYY-MM-DD
...
print(f"KICK_USER set: {bool(USERNAME)} | len: {len(USERNAME) if USERNAME else 0}")
print(f"KICK_PASS set: {bool(PASSWORD)} | len: {len(PASSWORD) if PASSWORD else 0}")

if not USERNAME or not PASSWORD:
    raise RuntimeError("Missing KICK_USER/KICK_PASS in environment")
